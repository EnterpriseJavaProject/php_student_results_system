...
$dob=$_POST['dob']; 
$status=$_POST['status'];
$sql="update admission_table set StudentName=:studentname,RollId=:roolid,StudentEmail=:studentemail,Gender=:gender,DOB=:dob,Status=:status where StudentId=:stid ";
...
$sql = "SELECT admission_table.StudentName,admission_table.RollId,admission_table.RegDate,admission_table.StudentId,admission_table.Status,admission_table.StudentEmail,admission_table.Gender,admission_table.DOB,courses.courses_name,courses.course_level from admission_table join courses on courses.id=admission_table.ClassId where admission_table.StudentId=:stid";
$query = $dbh->prepare($sql);
$query->bindParam(':stid',$stid,PDO::PARAM_STR);
$query->execute();
...
<div class="col-sm-10">
<input type="text" name="rollid" class="form-control" readonly="true" id="rollid" value="<?php echo htmlentities($result->RollId)?>" maxlength="5" required="required" autocomplete="off">
</div>
</div>
...
<?php }?>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="col-sm-10">
<input type="text" name="classname" class="form-control" id="classname" value="<?php echo htmlentities($result->courses_name)?>(<?php echo htmlentities($result->course_level)?>)" readonly>
                                                        </div>
                                                    </div>
<div class="form-group">
